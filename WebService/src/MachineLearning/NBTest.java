...
public class NBTest {
	...
	public static String test(String testTweets, String fileName) throws IOException {
		...
		while ((doc1=buf.readLine())!=null){
			...
			String key="";
			if(wordMap.length==2) key = wordMap[0];
			else key = wordMap[0]+","+wordMap[1]; 
		...
		for(String doc : docs) {
			...
			for (int j = 0; j < tokens.size(); j++) {
				...
				if(words.containsKey(tempT)){
					bad+= Math.log(1.0*(words.get(tempT)+1)/(words.get("0")+size));
				}else {
			...
			for (int j = 0; j < tokens.size(); j++) {
				...
				if(words.containsKey(tempT)){
					good+= Math.log(1.0*(words.get(tempT)+1)/(words.get("1")+size));
				}else {
